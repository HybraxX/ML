...
function setLanguage(lang = 'en') {
  ...
  document.querySelectorAll('[data-lang-key]').forEach(el => {
    ...
    if (translations[lang] && translations[lang][key]) {
      // preserve inline HTML for some nodes (like we used span inside)
...
if (langBtn) {
  ...
  langMenu.querySelectorAll("button").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      setLanguage(lang); // reuse your existing translation function
...
const OPENWEATHER_API_KEY = 'REPLACE_WITH_YOUR_OPENWEATHER_API_KEY';
...
/* -------------------------
   IoT SENSORS: fetching + UI
   ------------------------- */

// Change this to your real endpoint when available
...
function updateSensorUI(data) {
  ...
  safeSetHTML('soilMoistureValue', `${m}% <span id="soilMoistureStatus" class="text-lg font-medium">${m < 40 ? translations['en'].status_low : 'OK'}</span>`);
  safeSetText('soilMoistureUpdated', translations['en'].last_update);
  safeSetText('soilPhValue', Number(ph).toFixed(1));
  safeSetText('soilPhUpdated', translations['en'].last_update);
  safeSetText('airHumidityValue', `${ah}%`);
  safeSetText('airHumidityUpdated', translations['en'].last_update);
  safeSetText('waterLevelValue', `${wl}%`);
  safeSetText('waterLevelUpdated', translations['en'].last_update);
  safeSetText('airTempValue', `${at}°C`);
  safeSetText('airTempUpdated', translations['en'].last_update);

  safeSetText('speakerStatus', sp ? 'ON' : 'OFF');
  safeSetText('speakerUpdated', translations['en'].last_update);
  ...
  const nStatus = n > 60 ? translations['en'].status_good : (n > 40 ? translations['en'].status_fair || 'Fair' : translations['en'].status_low);
  const pStatus = p > 60 ? translations['en'].status_good : (p > 40 ? translations['en'].status_fair || 'Fair' : translations['en'].status_low);
  const kStatus = k > 60 ? translations['en'].status_good : (k > 40 ? translations['en'].status_fair || 'Fair' : translations['en'].status_low);
  ...
  // save latest for recommendations
  window._LATEST_SOIL = { moisture: m, ph: ph, temp: soilTemp, nitrogen: n, phosphorus: p, potassium: k, air_humidity: ah };
}

// Attempt to fetch from endpoint; fallback to mock
function fetchSensorData() {
  if (SENSOR_ENDPOINT === 'REPLACE_WITH_YOUR_SENSOR_ENDPOINT_OR_API') {
    // mocked sensor snapshot (real code should call your REST API)
    const mock = {
      soil_moisture: Math.floor(Math.random() * 40) + 30, // 30-70%
      ph: (Math.random() * 1.6 + 5.6).toFixed(1), // 5.6-7.2
  ...
  return fetch(SENSOR_ENDPOINT)
    ...
    .catch(err => {
      console.warn('Could not fetch sensor data; using mock.', err);
      const fallback = {
        soil_moisture: 38, ph: 6.4, soil_temp: 22, air_humidity: 62,
        nitrogen: 70, phosphorus: 55, potassium: 65
      };
...
// initial fetch + regular updates
fetchSensorData();
setInterval(fetchSensorData, 12000); // refresh every 12s
